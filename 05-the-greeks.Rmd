...
The only component from that list that we can't read off a market price is the implied volatility. Implied volatility is perhaps the most complicated idea in option pricing, and *vega* measures how changes in implied volatility affect the price of an option. Before getting to *vega*, we're going to spend a lot of time on implied volatility. It's the most important idea in all of options pricing, but it's not that complicated. The idea behind it is simple, and we want to make it as clear as can be. 

Those are the 5 greeks. It's essential that you learn them, because knowing them allows you to: 

  1. Construct options positions that have the risk-reward characteristics you want.
  2. Adjust your options to changing market conditions. 
  
Once we've armed ourselves with knowledge of the greeks, we'll look at two more options strategies - the straddle and the put spread - to show how to use the greeks to construct and manage options trades. 

That's all coming up. In this chapter, we'll start our journey by looking at *delta* and *gamma*. 

## Delta 

Here's a familiar payoff diagram - Apple's \$205 strike call. What does it tell us   about the call's *delta*?
