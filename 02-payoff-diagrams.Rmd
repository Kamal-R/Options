...
kableExtra:::save_kable(Apple_Call_Table, zoom = 3, 
                        file="img/02_Apple_Call_Table.png") 
img = image_read('img/02_Apple_Call_Table.png')
img = image_crop(img, "1500x900"); img
...
<figure style='margin-left:0%; margin-right:0%;'>
    ...
    <p> <img src="img/02_Apple_Call_Table.png"
         style='height:auto; width:auto; border:none; display:block;' /> </p>
</figure>
-->
...
kableExtra:::save_kable(Apple_Put_Table, zoom = 3, 
                        file = "img/02_Apple_Put_Table.png") 
...
kableExtra:::save_kable(Apple_Different_Strikes, zoom = 3, 
                        file = "img/02_Apple_Different_Strikes.png") 
img = image_read('img/02_Apple_Different_Strikes.png')
img = image_crop(img, "500x450"); img
```
...
<figure style='margin-left:10%;'>
    ...
    <p> <img src="img/02_Apple_Different_Strikes.png" 
         style='height:23%; width:45%; border:none; display:block;' /> </p>
</figure>
-->
...
kableExtra:::save_kable(Apple_Different_Strikes_Call_Payoff, zoom = 3, 
...
img = image_crop(img, "1500x750"); img
```
...
kableExtra:::save_kable(Apple_Different_Strikes_Call_Relative_Payoff,
                        zoom = 3, file = "img/02_Apple_Different_Strikes_Call_Relative_Payoff_Table.png") 
img = image_read('img/02_Apple_Different_Strikes_Call_Relative_Payoff_Table.png')
img = image_crop(img, "3000x2250"); img
```
...
The next thing to notice is that the price of an option varies wildly depending on its strike price. To understand why, we can decompose the price of an option into two components - time value and intrinsic value. The intrinsic value of an option is how much money the buyer would make if they exercised the option today. With Apple trading at `r currency(222.76)`, the `r currency(205)` call costs `r currency(23.63)` and the `r currency(205)` put costs `r currency(5.45)`. If we exercised the call today, we would make `r currency(222.76)` - `r currency(205)` = `r currency(222.76-205)`. This is the intrinsic value of the call. The remaining `r currency(23.76)` - `r currency(222.76-205)` =`r currency(23.76+222.76-205)` is the time value of the call. The put has no intrinstic value, because Apple's shares at `r currency(222.76)` are trading well above the put's strike price of `r currency(205)`. This means the entire `r currency(23.76+222.76-205)` cost of the put comes from its time value. 

The time value of an option slowly decays to `r currency(0)` as it approaches expiry, leaving only its intrinsic value. This distinction between time value and instrinsic value helps explain the wide variation in options prices in Table \@ref(tab:Apple-Different-Strikes-Call-Payoff-Table). The most extreme differences are in the `r currency(145)` strike call and `r currency(230)` strike put. The `r currency(145)` strike call is worth `r currency(78.45)` and the put is worth `r currency(0.32)`. With Apple trading at `r currency(222.76)`, the `r currency(78.45)` call has `r currency(222.76)` - `r currency(145)` = `r currency(222.76-145)` of intrinsic value and `r currency(78.45)` - `r currency(222.76-145)` = `r currency(78.45+222.76-145)` of time value. The `r currency(145)` put, in comparison, carries no intrinsic value and `r currency(0.32)` of time value. 
