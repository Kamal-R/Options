...
call_payoff <- function(data, main, Call_K, Call_cost) {
  ggplot() + 
  ...
  geom_segment(aes(x = Call_K+Call_cost, xend = Call_K+Call_cost, 
                   y = 0, yend = 0, color = "Breakeven"), 
                 show.legend = FALSE, linetype = "solid") +
  annotate(geom = "text", x = Call_K+Call_cost+1, y = 0, color = "black", hjust = 0,
           label = paste0("Breakeven: $",Call_K + Call_cost), size = 5 ) + 
  scale_y_continuous(label = scales::dollar) +
  scale_x_continuous(label = scales::dollar) + 
  labs(title = main,
       x = "Stock Price",
       y = "Profit Per Share") +
  scale_color_manual(values = c("Breakeven" = plot_fg, "Profit" = "green", "Loss" = "red"),
                     labels = c("", "", ""),
                     guide = "legend") + 
...
put_payoff <- function(data, main) {
  ...
  geom_segment(data = data,
               ...
               show.legend = FALSE, col = "red") + 
  geom_hline(aes(yintercept = 0, color = "Breakeven"),
            linetype = "dashed") +
  geom_vline(aes(xintercept = Put_K - Put_cost, color = "Breakeven"), 
  ...
  labs(title = main,
       ...
       y = "Profit Per Share") +
  scale_color_manual(values = c("Breakeven" = plot_fg, "Profit" = "green", "Loss" = "red"),
                     labels = c(paste0("Breakeven: ", currency(Put_K - Put_cost)), "", ""),
                     guide = "legend") + 
  guides(color = guide_legend(override.aes = list(color = c(plot_fg, "white","white"),
                                                  linetype = c("dashed","solid","solid")))) + 
...
call_payoff(data = Call_Payoff_XYZ_230, Call_K = Call_K, Call_cost = Call_cost,
...
Put_Payoff_XYZ_180 <- tibble( "Put_x" = seq(from = x_min, to = x_max, by = 0.01),
                           ...
                           "Profitable" = ifelse(-Put_cost - (Put_x < Put_K) * (Put_x - Put_K) > 0,
                                                 "Profit", "Loss") )
put_payoff(data = Put_Payoff_XYZ_180, main = "Buy: Company XYZ's $180 Put")
```     
...
Call_Payoff_Apple_205 <- tibble( "Call_x" = seq(from = x_min, to = x_max, by = 0.01),
                              ...
                              "Profitable" = ifelse(-Call_cost + (Call_x > Call_K)*(Call_x-Call_K) > 0, 
                                                    "Profit","Loss") )
call_payoff(data = Call_Payoff_Apple_205, main = "Buy: Apple's $205 Strike Call",
            Call_K = Call_K, Call_cost = Call_cost)
```
...
if (save_tables_as_pictures) { save_kable(Apple_Call_Table, file="img/02_Apple_Call_Table.png") }
...
Put_Payoff_Apple_205 <- tibble( "Put_x" = seq(from = x_min, to = x_max, by = 0.01),
                                ...
                                "Profitable" = ifelse(-Put_cost - (Put_x < Put_K) * (Put_x - Put_K) > 0,
                                                      "Profit", "Loss") )
put_payoff(data = Put_Payoff_Apple_205, main = "Buy: Apple's $205 Strike Put")
...
if (save_tables_as_pictures) { save_kable(Apple_Put_Table, file="img/02_Apple_Put_Table.png") }
...
if (save_tables_as_pictures) { save_kable(Apple_Different_Strikes, file="img/02_Apple_Different_Strikes.png") }
if (load_table_pictures) { knitr::include_graphics(path="img/02_Apple_Call_Table.png") }
```
...
if (save_tables_as_pictures) { save_kable(Apple_Different_Strikes_Call_Payoff,
...
if (save_tables_as_pictures) { save_kable(Apple_Different_Strikes_Call_Relative_Payoff,
