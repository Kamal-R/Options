...
call_payoff <- function(data, main, Call_K, Call_cost) {
  ...
  geom_segment(aes(x = Call_K+Call_cost, xend = Call_K+Call_cost, 
                   y = 0, yend = 0, color = "Breakeven"), 
                 show.legend = FALSE, linetype = "solid") +
  annotate(geom = "text", x = Call_K+Call_cost+5, y = 0, color = "black",
           label = paste0("Breakeven: $",Call_K + Call_cost) ) + 
...
# hide_legend(ggplotly(call_payoff(data = Call_Payoff_XYZ_230, Call_K = Call_K, Call_cost = Call_cost,
#                                  main = "Buy: Company XYZ's $230 Strike Call"))) %>% 
