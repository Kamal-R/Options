...
options_theme <- function(legend_position="left") {
  theme_classic() + 
...
stock_payoff <- function(data, main, stock_cost, breakeven_offset = 2) {
  ggplot() + 
...
call_payoff <- function(data, main, Call_K, Call_cost, breakeven_offset = 2) {
  ggplot() + 
...
stock_and_call_payoff <- function(data_stock, data_call, main, stock_cost, Call_K, Call_cost) { 
  ggplot() + 
  ...
  geom_line(data = data_call, aes(Call_x, Call_y, color = Profitable),
            linetype = "solid") +   
...
put_payoff <- function(data, main, breakeven_offset = 15) {
  ggplot() + 
  geom_line(data = data, aes(Put_x, Put_y, color = Profitable), 
            linetype = "solid") +
  geom_segment(data = data,
               aes(x = min(Put_x), xend = min(Put_x)-1, y = max(Put_y), yend = max(Put_y)+1), 
               arrow = arrow(length = unit(0.50, "cm"), type = "open"), 
               show.legend = FALSE, col = "green") + 
  geom_segment(data = data,
               aes(x = max(Put_x), xend = max(Put_x), y = min(Put_y), yend = min(Put_y)), 
               arrow = arrow(length = unit(0.50, "cm"), angle = 210, type = "open"), 
               show.legend = FALSE, col = "red") + 
  annotate(geom = "text", x = Put_K-Put_cost-breakeven_offset, y = 0, color = "black", hjust = 0,
           label = paste0("Breakeven: ",scales::dollar(Put_K-Put_cost)), size = 5 ) + 
  scale_y_continuous(label = scales::dollar) +
  scale_x_continuous(label = scales::dollar) + 
  labs(title = main, x = "Stock Price", y = "Profit") +
  scale_color_manual(values = c("Breakeven" = plot_fg, "Profit" = "green3", "Loss" = "red"),
                     labels = c("", "", ""), guide = "legend") + 
  guides(color = guide_legend(override.aes = list(color = c("white", "white","white"),
                                                  linetype = c("dashed","solid","solid")))) + 
  options_theme(legend_position="right")
}
```
...
b1 <- bracketsGrob(0.25, 0.25, 0.25, 0.5, lwd=2, col="red")
b2 <- bracketsGrob(0.75, 0.75, 0.75, 0.5, lwd=2, col="green3")

### Source: https://stackoverflow.com/questions/35633239/add-curly-braces-to-ggplot2-and-then-use-ggsave
temp <- stock_payoff(Stock_Payoff_XYZ, "Buy: XYZ's Shares at $200", Stock_cost, 5)
...
b1 <- bracketsGrob(0.25, 0.075, 0.25, 0.24, lwd=2, col="red")
b2 <- bracketsGrob(0.80, 0.69, 0.80, 0.24, lwd=2, col="green3")

temp <- call_payoff(data = Call_Payoff_XYZ_200, Call_K = Call_K, Call_cost = Call_cost,
                    main = "Buy: XYZ's $200 Strike Call")
y_axis_labels <- na.omit( ggplot_build(temp)$layout$panel_params[[1]]$y$get_labels() )
y_axis_labels <- as.numeric(gsub('\\(','-',gsub('[)$,]', '', y_axis_labels)))
...
temp <- temp + theme( axis.text.y = element_markdown(colour = tick_color) ) + 
     ...
     annotate( geom = "text", x = 270, y = 20.5, color = "green3", hjust = 0, 
               label = paste0("Profits"), size = 5 ) + 
     annotate( geom = "text", x = Call_K+Call_cost-14, y = min(y_axis_labels)-17.5, color = "green3", hjust = 0,
               label = paste0("Breakeven: ", scales::dollar(Call_K+Call_cost)), size = 4 ) + 
     annotate( geom = "text", x = Call_K+Call_cost-14, y = min(y_axis_labels)-17.5, color = "red", hjust = 0,
               label = "Breakev", size = 4 ) +
     annotation_custom( b1 ) + annotation_custom( b2 ) 
temp; rm(temp)
...
temp <- stock_and_call_payoff( data_stock = Stock_Payoff_XYZ, data_call = Call_Payoff_XYZ_200, 
                               main = "XYZ's Shares vs. $200 Strike Calls", 
                               stock_cost = Stock_cost, Call_K = Call_K, Call_cost = Call_cost ) 
y_axis_labels <- na.omit( ggplot_build(temp)$layout$panel_params[[1]]$y$get_labels() )
y_axis_labels <- as.numeric(gsub('\\(','-',gsub('[)$,]', '', y_axis_labels)))
tick_color <- ifelse( as.numeric(gsub('\\(','-',gsub('[)$,]', '', y_axis_labels))) < 0,
                      "red", "green3" )
...
temp <- put_payoff(data = Put_Payoff_XYZ_180, main = "Buy: XYZ's $180 Put")
y_axis_labels <- na.omit( ggplot_build(temp)$layout$panel_params[[1]]$y$get_labels() )
tick_color <- ifelse( as.numeric(gsub('\\(','-',gsub('[)$,]', '', y_axis_labels))) < 0,
                      "red", "green3" )
temp + theme( axis.text.y = element_markdown(colour = tick_color) ); rm(temp)
```     
...
temp <- call_payoff(data = Call_Payoff_Apple_205, Call_K = Call_K, Call_cost = Call_cost,
                    main = "Buy: Apple's $205 Strike Call")
y_axis_labels <- na.omit( ggplot_build(temp)$layout$panel_params[[1]]$y$get_labels() )
tick_color <- ifelse( as.numeric(gsub('\\(','-',gsub('[)$,]', '', y_axis_labels))) < 0,
                      "red", "green3" )
temp + theme( axis.text.y = element_markdown(colour = tick_color) ); rm(temp)
...
```{r Apple-Call-Table, tab = NULL, results = 'hide'}
...
Apple_Call_Table <- Apple_Call_Table %>% 
  ...
  knitr::kable(booktabs = TRUE, escape = FALSE, label = NULL, 
               caption = "Returns: 100 Shares vs. 1 Call") %>% 
...
```{r Apple-Call-Table-Format, out.width = out_width_table, fig.align = "center", tab = NULL}
if (save_tables_as_pictures) { save_kable(Apple_Call_Table, zoom = 2.5,
                                          file="img/02_Apple_Call_Table.png") }
if (load_table_pictures) { knitr::include_graphics(path="img/02_Apple_Call_Table.png") }
```
...
temp <- put_payoff(data = Put_Payoff_Apple_205, main = "Buy: Apple's $205 Strike Put")
y_axis_labels <- na.omit( ggplot_build(temp)$layout$panel_params[[1]]$y$get_labels() )
tick_color <- ifelse( as.numeric(gsub('\\(','-',gsub('[)$,]', '', y_axis_labels))) < 0,
                      "red", "green3" )
temp + theme( axis.text.y = element_markdown(colour = tick_color) ); rm(temp)
...
Apple_Put_Table <- Apple_Put_Table %>% 
  ...
  knitr::kable(booktabs = TRUE, escape = FALSE,
               caption = "Returns: 100 Shares vs. 1 Put") %>% 
  kable_styling(bootstrap_options = c("bordered", "condensed", "responsive"),
                full_width = FALSE ) %>%
  add_header_above(c("Stock" = 1, "Own 100 Shares" = 2, 
...
```{r Apple-Put-Table-Format, out.width = out_width_table, fig.align = "center", tab = NULL}
if (save_tables_as_pictures) { save_kable(Apple_Put_Table, zoom = 2.5,
                                          file = "img/02_Apple_Put_Table.png") }
if (load_table_pictures) { knitr::include_graphics(path="img/02_Apple_Put_Table.png") }
```
...
Apple_Different_Strikes <- knitr::kable(Apple_Different_Strikes, 
             caption = "Cost on Sept. 11, 2019") %>%
kable_styling( bootstrap_options = c("bordered", "condensed", "responsive"),
               full_width = FALSE )
```

```{r Apple-Different-Strikes-Table-Format, out.width = out_width_table, fig.align = "center", tab = NULL}
if (save_tables_as_pictures) { save_kable(Apple_Different_Strikes, zoom = 2.5,
...
```{r Apple-Different-Strikes-Call-Payoff-Table-Format, out.width = out_width_table, fig.align = "center", tab = NULL}
if (save_tables_as_pictures) { save_kable(Apple_Different_Strikes_Call_Payoff, zoom = 2.5,
                                          file = "img/02_Apple_Different_Strikes_Call_Payoff_Table.png") }
if (load_table_pictures) {knitr::include_graphics(path="img/02_Apple_Different_Strikes_Call_Payoff_Table.png")}
```
...
```{r Apple-Different-Strikes-Call-Relative-Payoff-Table-Format, out.width=out_width_table, fig.align = "center", tab = NULL}
if (save_tables_as_pictures) { save_kable(Apple_Different_Strikes_Call_Relative_Payoff, zoom = 2.5,
                                          file="img/02_Apple_Different_Strikes_Call_Relative_Payoff_Table.png") }
if (load_table_pictures) {knitr::include_graphics(path="img/02_Apple_Different_Strikes_Call_Relative_Payoff_Table.png")}
```
