...
stock_payoff <- function(data, main, stock_cost, breakeven_offset = 2) {
  ...
  annotate(geom = "text", x = stock_cost+breakeven_offset, y = 0, color = "black", hjust = 0,
           label = paste0("Breakeven: ",scales::dollar(stock_cost)), size = 5 ) + 
...
call_payoff <- function(data, main, Call_K, Call_cost, breakeven_offset = 2) {
  ...
  geom_segment(data = data,
               ...
               arrow = arrow(length = unit(0.50, "cm"), angle = 30, type = "open"), 
               show.legend = FALSE, col = "green3") + 
  geom_segment(aes(x = Call_K+Call_cost, xend = Call_K+Call_cost, 
                   y = 0, yend = 0, color = "Breakeven"), 
               show.legend = FALSE, linetype = "solid") +
  geom_segment(aes(x = Call_K+Call_cost, xend = Call_K+Call_cost, 
                   y = 0, yend = 0, color = "Breakeven"), 
                 show.legend = FALSE, linetype = "solid") +
  annotate(geom = "text", x = Call_K+Call_cost+breakeven_offset, y = 0, color = "black", hjust = 0,
           label = paste0("Breakeven: ",scales::dollar(Call_K + Call_cost)), size = 5 ) + 
  scale_y_continuous(label = scales::dollar) +
  scale_x_continuous(label = scales::dollar) + 
  labs(title = main, x = "Stock Price", y = "Profit") +
...
Call_Payoff_XYZ_230 <- tibble( "Call_x" = seq(from = x_min, to = x_max, by = 0.01),
                               "Call_y" = -Call_cost + (Call_x > Call_K)*(Call_x-Call_K),
                               "Profitable" = ifelse(-Call_cost + (Call_x > Call_K)*(Call_x-Call_K) > 0, 
                                                      "Profit","Loss") )

temp <- call_payoff(data = Call_Payoff_XYZ_230, Call_K = Call_K, Call_cost = Call_cost,
                    main = "Buy: XYZ's $230 Strike Call")
y_axis_labels <- na.omit( ggplot_build(temp)$layout$panel_params[[1]]$y$get_labels() )
tick_color <- ifelse( as.numeric(gsub('\\(','-',gsub('[)$,]', '', y_axis_labels))) < 0,
                      "red", "green3" )
temp + theme( axis.text.y = element_markdown(colour = tick_color) ); rm(temp)
