...
## Call Options
```{r Call-Option-Payoff-Plot, echo=FALSE, fig.fullwidth = TRUE, fig.align = "center", message = FALSE}
Call_cost <- 5
Call_K <- 230
x_min <- 210; x_max = 250
...
```{r Put-Option-Payoff-Plot, echo=FALSE, fig.fullwidth = TRUE, fig.align = "center", message = FALSE}
Put_cost <- 5
Put_K <- 180
x_min <- 150; x_max = 200
...
```{r Apple-Call-Option_Payoff, echo=FALSE, fig.fullwidth = TRUE, fig.align = "center", message = FALSE}
Call_cost <- 6.84
Call_K <- 205
Current_price <- 205.44
x_min = 180; x_max = 225
...
```{r Apple-Put-Option-Payoff, echo=FALSE, fig.fullwidth = TRUE, fig.align = "center", message = FALSE}
Apple_Put_Cost <- 6.10
Apple_Put_K <- 205
x_min = 180; x_max = 230
...
```{r Apple_205_Strike, echo = FALSE, fig.fullwidth = TRUE, fig.align = "center", message = FALSE}
...
The `r currency(Apple_Prices_K[2])` strike call expires worthless if the shares are below `r currency(Apple_Prices_K[2])`, loses money when they are below the breakeven of \$180 + \$44.85 = \$224.85, and makes money when shares are above $224.85. The \$140 strike call expires worthless when shares are below \$140, loses money when shares are below the breakeven of \$140 + \$78.45 = \$218.45, and makes money when they are above \$218.45. The shareholder column is straightforward - their return is how much the stock moves by between September 11, 2019 and December 20, 2019. 

This table can give us even more useful insights. By comparing all the values in a column, we can see that the dollar returns of one \$145 strike call are always within \$100 dollars of 100 shares. The dollar returns of the \$180 strike call are always within \$200 dollars of owning 100 shares, except when the shares are priced at \$150 (where the \$180 call loses significantly less money). The \$205 and \$230 strike calls have sufficiently different behavior, essentally because their breakeven is higher. However, they have much lower maximum losses and achieve similar dollar returns after shares climb above their respective breakeven prices. 
