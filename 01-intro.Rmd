...
```{r Apple-Call-Option_Payoff, echo=FALSE, fig.height = 4, fig.width = 6, fig.align = "center"}
Call_cost <- 6.84
Call_K <- 205
Current_price <- 205.44
x_min = 180; x_max = 225
...
``` {r Apple_Call_Table, echo = FALSE}
Apple_Current_Price <- 205.44
Apple_Call_K <- 205
Apple_Call_Cost <- 6.84
Apple_Stock_Price <- c( 150, 180, 184.90, 198.90, 211.84, 218.68, 225.98, 240 )

Apple_Shares_Percent_Return <- (Apple_Stock_Price - Apple_Current_Price) / Apple_Current_Price
Apple_Shares_Dollar_Return <- 100 * (Apple_Stock_Price - Apple_Current_Price)

Apple_Calls_Percent_Return <- 
  ( pmax( 0, (Apple_Stock_Price - Apple_Call_K ) ) - Apple_Call_Cost ) / Apple_Call_Cost 
Apple_Calls_Dollar_Return <- 100  * ( pmax( 0, (Apple_Stock_Price - Apple_Call_K ) ) - Apple_Call_Cost )
...
```
TThe buyer can make a ton of money with this call. To double his investment, he only needs to recover the `r currency(Apple_Call_Cost)` it cost to buy the call (done at the breakeven of `r currency(Apple_Call_K)` + `r currency(Apple_Call_Cost)` = `r currency(Apple_Call_K+Apple_Call_Cost)`), and have the shares go up another `r currency(Apple_Call_Cost)` to `r currency(Apple_Call_K+Apple_Call_Cost)` + `r currency(Apple_Call_Cost)` = `r currency(Apple_Call_K+2*Apple_Call_Cost)`. Remember that Apple was trading for `r currency(Apple_Current_Price)` when he bought the call. If he bought shares instead, the buyer would make \dfrac{`r paste(currency(Apple_Call_K+2*Apple_Call_Cost), currency(Apple_Current_Price), sep = " - ")`}{`r currency(Apple_Current_Price)`} = `r percent(2*Apple_Call_Cost/Apple_Current_Price)` on his investment. By using calls, he makes (`r currency(Apple_Call_K+2*Apple_Call_Cost)` - `r currency(Apple_Call_K)` - `r currency(Apple_Call_Cost)`) / `r currency(Apple_Call_Cost)` = `r percent(Apple_Call_Cost/Apple_Call_Cost)`. `r percent(Apple_Call_Cost/Apple_Call_Cost)` seems so much larger than `r percent(2*Apple_Call_Cost/Apple_Current_Price)` that this seems impossible, but the way to think about this is by looking at how much of the upside in the stock the call buyer participates in. 

<!-- These percentges (and multiples) are from the table - we can view the above table in HTML to confirm them-->
Compared to someone who owns 100 shares of Apple, the call buyer participates in 51.66\% of the gains when Apple shares are worth `r currency(Apple_Call_K+2*Apple_Call_Cost)` at expiry. This goes up to 68.84\% when the shares are worth `r currency(Apple_Shares_Percent_Return[7]*Apple_Current_Price)` and \$81.48\% when they are worth `r currency(Apple_Shares_Percent_Return[8]*Apple_Current_Price)`. His participation increases as Apple's shares go up because he must recover the cost of the call before beginning to earn a profit. This happens at his breakeven of `r currency(Apple_Call_K)` + `r currency(Apple_Call_Cost)` = `r currency(Apple_Call_K+Apple_Call_Cost)`, after which the call buyer gets dollar-for-dollar partcipation in the upside of the stock.

Testing inline: <span class="fraction" data-top="`r currency(Apple_Call_Strike_Price+2*Apple_Call_Cost)` - `r currency(Apple_Share_Price)`" data-bottom="`r currency(Apple_Share_Price)`"></span>. Testing. 
