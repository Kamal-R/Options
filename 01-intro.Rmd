...
```{r Apple-Put-Option-Payoff, echo=FALSE, fig.height = 4, fig.width = 6, fig.align = "center"}
Apple_Put_Cost <- 6.10
Apple_Put_K <- 205
x_min = 180; x_max = 230
...
Apple_Put_Table <- tibble( "Stock Price" = currency(Apple_Stock_Prices),
                           ...
                           "Calls/Shares" = sign(Apple_Puts_Dollar_Return) * 
...
```
For now, let’s look at how owning Apple stock compares to holding the put to expiry. If Apple’s stock increases by 10% (from `r currency(Apple_Current_Price)` to `r currency(1.10*Apple_Current_Price)`), a shareholder will make `r currency(1.10*Apple_Current_Price)` - `r currency(Apple_Current_Price)` = `r currency(1.10*Apple_Current_Price-Apple_Current_Price)` per share. If Apple’s stock declines 10\% (from `r currency(Apple_Current_Price)` to `r currency(0.90*Apple_Current_Price)`), a shareholder loses `r currency(0.90*Apple_Current_Price)` - `r currency(Apple_Current_Price)` = `r currency(0.90*Apple_Current_Price-Apple_Current_Price)` per share. Now, let’s look at the put. It costs `r currency(Apple_Put_Cost)` per share, so if Apple’s shares stay above `r currency(Apple_Put_K)`, the put buyer loses `r currency(Apple_Put_Cost)` per share. If Apple’s stock falls to \$184.90, the put buyer makes \$205 - \$184.90 - \$6.84 = \$14.00 per share, which is about \$14.00 / \$6.10 = 2.3 times (or 230\%) on his investment. This percentage may seem too big to be true but once again, the way to think about it is through how much participation a put gives it buyer to the downside of a stock (which is the upside of the put). 

The put buyer loses \$610 whenever the shares are above \$205 while the shareholder makes between 3.12\% to 16.82\%. When the shares are below the put buyer's breakeven of \$198.90, the put buyer makes money when the shareholder loses money. When the shares are at \$184.90, the put buyer captures 68.16\% of the downside of the stock. When the shares are at \$180, the put buyer captures 74.29\%; when the shares are \$150, the put buyer captures 88.20\%. This percentage gradually increases because the put buyer must recover the cost of buying the put before he can begin to earn profits.
