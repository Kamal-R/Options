...
We can do a lot more with options than just buying or selling one put or one call. Stocks, calls, and puts are the basic building blocks that we can combine to construct investments with any payoff structure we want. There are tons of options strategies, and they're all just combinations of these building blocks. You don't need to know their names to understand their strengths and weaknesses, and the goal of this chapter is to show you how to construct options strategies on-the-fly that have the payoff that you want. In the next few chapters, we'll be getting to the greeks, which will let us break down their risk exposures. For now, let's nail down how to construct their their payoff diagrams.

## Protective Put

A protective put combines owning shares of a stock with buying puts on the same stock. The goal of this strategy is protect the stock position against large losses. The payoff diagram has two dotted lines 
...
fill_color_right <- data.frame(xstart = 120, xend = 200, col = "white")
fill_color_left <- data.frame(xstart = 200, xend = 260, col = "white")
...
temp <- temp + theme( axis.text.y = element_markdown(colour = tick_color) ) +
     ...
     annotate( geom = "text", x = 180-16-4, y = -7.5, color = "gray50", hjust = 0,
               label = paste0("$164"), size = 3.5 ) +
     annotate("point", x = 208, y = 0, colour = "gray65") + 
     annotate( geom = "text", x = 208-4, y = -7.5, color = "gray65", hjust = 0,
               label = paste0("$208"), size = 3.5 ) +
...
temp <- temp + theme( axis.text.y = element_markdown(colour = tick_color) ) +
     ...
     annotate( geom = "text", x = 180-16-4, y = -7.5, color = "gray65", hjust = 0,
               label = paste0("$164"), size = 3.5 ) +
     annotate("point", x = 208, y = 0, colour = "gray50") + 
     annotate( geom = "text", x = 208-4, y = -7.5, color = "gray50", hjust = 0,
               label = paste0("$208"), size = 3.5 ) +
