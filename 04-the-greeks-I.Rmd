...
Call_price_on_expiry <- sapply(Call_x, function(x) { bscall(s = x, k = 200, v = 0.16, r = 0.02, tt = 0, d = 0) })
...
for (ii in 1:length(Call_price_on_expiry)) { 
  Call_delta_on_expiry[ii] <- greeks( bscall(s = x_min-0.1+0.1*ii, k = 200, v = 0.16, 
                                   r = 0.02, tt = 0, d = 0) )[2] 
}

Call_price_3months_expiry <- sapply(Call_x, function(x) { bscall(s = x, k = 200, v = 0.16, r = 0.02, tt = 0.25, d = 0) })
...
for (ii in 1:length(Call_price_3months_expiry)) { 
  Call_delta_3months_expiry[ii] <- greeks( bscall(s = x_min-0.1+0.1*ii, k = 200, v = 0.16, 
                                   r = 0.02, tt = 0.25, d = 0) )[2] 
}

Call_price_6months_expiry <- sapply(Call_x, function(x) { bscall(s = x, k = 200, v = 0.16, r = 0.02, tt = 0.5, d = 0) })
...
for (ii in 1:length(Call_price_6months_expiry)) { 
  Call_delta_6months_expiry[ii] <- greeks( bscall(s = x_min-0.1+0.1*ii, k = 200, v = 0.16, 
                                   r = 0.02, tt = 0.5, d = 0) )[2] 
}
...
for (ii in 1:length(Call_price_12months_expiry)) { 
  Call_delta_12months_expiry[ii] <- greeks( bscall(s = x_min-0.1+0.1*ii, k = 200, v = 0.16, 
                                   r = 0.02, tt = 1, d = 0) )[2] 
}
...
Call_payoff_on_expiry <- Call_price_on_expiry - Call_cost_on_expiry
Call_delta_on_expiry <- vector(length = length(Call_price_on_expiry)) 
for (ii in 1:length(Call_delta_on_expiry)) { 
  Call_delta_on_expiry[ii] <- greeks( bscall(s = x_min-0.1+0.1*ii, k = 200, v = 0.16, 
                                             r = 0.02, tt = 0, d = 0) )[2] 
}
...
Call_payoff_3months_expiry <- Call_price_3months_expiry - Call_cost_3months_expiry
Call_delta_3months_expiry <- vector(length = length(Call_price_3months_expiry)) 
for (ii in 1:length(Call_delta_3months_expiry)) { 
  Call_delta_3months_expiry[ii] <- greeks( bscall(s = x_min-0.1+0.1*ii, k = 200, v = 0.16, 
                                   r = 0.02, tt = 0.25, d = 0) )[2] 
}
Call_gamma_3months_expiry <- vector(length = length(Call_delta_on_expiry)) 
for (ii in 1:length(Call_gamma_3months_expiry)) { 
  Call_gamma_3months_expiry[ii] <- greeks( bscall(s = x_min-0.1+0.1*ii, k = 200, v = 0.16, 
...
Call_payoff_12months_expiry <- Call_price_12months_expiry - Call_cost_12months_expiry
Call_delta_12months_expiry <- vector(length = length(Call_price_12months_expiry)) 
for (ii in 1:length(Call_delta_12months_expiry)) { 
  Call_delta_12months_expiry[ii] <- greeks( bscall(s = x_min-0.1+0.1*ii, k = 200, v = 0.16, 
                                   r = 0.02, tt = 1, d = 0) )[2] 
}
Call_gamma_12months_expiry <- vector(length = length(Call_delta_on_expiry)) 
for (ii in 1:length(Call_gamma_12months_expiry)) { 
  Call_gamma_12months_expiry[ii] <- greeks( bscall(s = x_min-0.1+0.1*ii, k = 200, v = 0.16, 
                                   r = 0.02, tt = 1, d = 0) )[3] 
}
...
Call_gamma_on_expiry <- vector(length = length(Call_delta_on_expiry)) 
for (ii in 1:length(Call_gamma_on_expiry)) { 
  Call_gamma_on_expiry[ii] <- greeks( bscall(s = x_min-0.1+0.1*ii, k = 200, v = 0.16, 
...
Call_gamma_3months_expiry <- vector(length = length(Call_delta_3months_expiry)) 
for (ii in 1:length(Call_price_3months_expiry)) { 
  Call_gamma_3months_expiry[ii] <- greeks( bscall(s = x_min-0.1+0.1*ii, k = 200, v = 0.16, 
                                   r = 0.02, tt = 0.25, d = 0) )[3] 
}
...
Call_gamma_6months_expiry <- vector(length = length(Call_delta_6months_expiry)) 
for (ii in 1:length(Call_delta_6months_expiry)) { 
  Call_gamma_6months_expiry[ii] <- greeks( bscall(s = x_min-0.1+0.1*ii, k = 200, v = 0.16, 
                                   r = 0.02, tt = 0.5, d = 0) )[3] 
}
...
Call_gamma_12months_expiry <- vector(length = length(Call_delta_12months_expiry)) 
for (ii in 1:length(Call_delta_6months_expiry)) { 
  Call_gamma_12months_expiry[ii] <- greeks( bscall(s = x_min-0.1+0.1*ii, k = 200, v = 0.16, 
                                   r = 0.02, tt = 1, d = 0) )[3] 
}
