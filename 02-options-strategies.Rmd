...
ggplot() + 
  geom_line(data = Call_Payoff_Plot, aes(Call_x, Covered_Call_y, color = "Shares"), linetype = "solid") +
  geom_line(data = Call_Payoff_Plot, aes(Call_x, rep(0,length(Call_x)), color = "Breakeven"), linetype = "dashed") +
  geom_line(data = Call_Payoff_Plot, aes(x = Apple_Current_Price - Apple_Call_Cost, 
                                         ...
                                         color = "Breakeven"),
                                         linetype = "dashed") +
  geom_line(data = Call_Payoff_Plot, aes(Call_x, Call_x - Apple_Current_Price, color = "Own_Shares"), linetype = "solid") +
  scale_y_continuous(label = scales::dollar) +
  scale_x_continuous(label = scales::dollar) + 
...
```{r Apple-Options-Cost, echo = FALSE}
...
ggplot() + 
  ...
  geom_line(data = Put_Payoff_Plot, aes(x = Apple_Current_Price + Apple_Put_Cost, 
                                         ...
                                         color = "Breakeven"),
                                         linetype = "dashed") +
  geom_line(data = Put_Payoff_Plot, aes(Put_x, Put_x - Apple_Current_Price, color = "Own_Shares"), linetype = "solid") +
  scale_y_continuous(label = scales::dollar) +
  scale_x_continuous(label = scales::dollar) + 
...
Apple_Protective_Put <- tibble( "Share Price" = scales::dollar(Apple_Future_Prices, negative_parens = TRUE),
                                ...
                                "Buy 50 Shares" = scales::dollar(50*(Apple_Future_Prices - Apple_Current_Price), 
                                                                 negative_parens = TRUE),
                                "Buy 100 Shares, Buy <br> 1 $205 Strike Put" = 
                                  scales::dollar( 100*(-Apple_Puts_Cost[3] + 
                                                         (Apple_Future_Prices - Apple_Current_Price) + 
                                                         pmax(0, Apple_Put_K - Apple_Future_Prices)), negative_parens = TRUE) )
...
```{r NLY-Protective-Put, echo = FALSE}
NLY_Put_Cost <- 0.66
NLY_Put_K <- 9
...
```{r NLY-Protective-Put-Plot, echo = FALSE, out.width = "80%", fig.align = "center", message = FALSE}
x_min <- 7; x_max <- 11
...
ggplot() + 
  ...
  scale_color_manual(values = c('Shares'=plot_fg,'Breakeven'=plot_fg,
                     ...
                     labels = c("Protective Put - Profit Per Share (with dividends)",
                                ...
                                paste0("Breakeven Price = ", currency(NLY_Current_Price + NLY_Put_Cost - 2*NLY_Div)) ),
                     guide = "legend") + 
  guides(color = guide_legend(override.aes = list(linetype = c("solid", "solid", "dashed")))) + 
  theme_classic() + 
  theme(plot.title = element_text(hjust = 0.5, face="bold", 
...
# 
# plot(x = Put_x, y = -NLY_Put_Cost + ( Put_x - NLY_Current_Price ) + pmax(0, NLY_Put_K - Put_x) + 2*NLY_Div, 
...
# lines(x = Put_x, y = Put_x - NLY_Current_Price - 2*NLY_Div, col = "orange", lwd = 3)
# 
# abline(v = NLY_Current_Price + NLY_Put_Cost - 2*NLY_Div, lty = 2, lwd = 1)
# abline(h = 0, lty = 2, lwd = 1)
# 
# lines(x = Put_x, y = Put_x)
# 
# legend("topleft", c("Protective Put - Profit Per Share (with dividends)",
#                     paste0("Breakeven Price = $", NLY_Current_Price + NLY_Put_Cost - 2*NLY_Div),
...
The goal of this section on protective puts is to highlight the advantages that using them can give investors who are trying to manage the risk of their investments. Protective puts allow investors to define the maximum loss they are willing to tolerate on an investment, which has practical and psychological benefits. Depending on market pricing, protective puts can also allow investors to construct trades with attractive risk-reward characteristics, like the NLY trade. Hopefully, this section has convinced you that protective puts are a powerful risk management tool to have in your investing toolkit. 
